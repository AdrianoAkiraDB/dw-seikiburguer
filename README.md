...
Recentemente, o Data Warehouse passou por uma grande evolução arquitetural para suportar regras reais de negócio, programas de fidelidade e automação de ingestão:

### 1. Modelagem de Clientes e Resolução de Identidade (Customer 360)
- **Preservação de LTV Histórico:** Adição da coluna `gasto_historico_acumulado` na `dim_clientes` para guardar o saldo legado dos clientes antes da migração para o DW, permitindo que clientes fiéis já entrem no programa de fidelidade nos níveis corretos (Bronze até Diamante) sem perda de histórico.
  
- **Normalização de Endereço:** Padronização dos campos para `rua` (logradouro + número) e `complemento` (bloco, apto), facilitando a integração futura com APIs de mapas para cálculo de frete por raio em KM.
  
- **Criação da `dim_clientes_telefones` (Relação 1:N):** Tabela criada para permitir múltiplos contatos (ex: cônjuge/filhos) associados à mesma conta titular. Possui índice de alta performance no telefone para viabilizar consultas em milissegundos por robôs de automação (n8n/WhatsApp).
  
- **Gestão de Identidade (`GENERATED BY DEFAULT`):** Configuração que permite a importação de IDs manuais legados (1 a 700) e geração automática e transparente de novos IDs (701 em diante) para futuros cadastros.

### 2. Motor de Ingestão e Processamento Atômico (Camada Bronze -> Silver)
- **Padronização da Staging (`stg_vendas_raw`):** Alinhamento da tabela de pouso com o layout operacional da planilha semanal (15 colunas atômicas), permitindo carga simplificada via CSV.
  
- **Desenvolvimento da Stored Procedure (`prc_processar_vendas_semana`):** Rotina transacional em PL/pgSQL que executa o pipeline completo de fechamento de vendas:
  
  1. *Idempotência:* Gravação segura em `fct_vendas` ignorando pedidos já processados para evitar duplicação de faturamento.
     
  2. *Snapshot de Custo:* Gravação dos itens em `fct_itens_pedido` congelando o custo de produção histórico daquele dia.
     
  3. *Ledger de Cashback:* Lançamento automático de débito de saldo utilizado e crédito de 5% sobre o valor líquido pago (com validade de 90 dias).
